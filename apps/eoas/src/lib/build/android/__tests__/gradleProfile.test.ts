import { BuildPhaseResult, LogMarker } from '@expo/eas-build-job';
...
describe('Gradle profile logs', () => {
  ...
  async function readProfileEvents(): Promise<BuildLogEvent[]> {
    ...
    try {
      await log.runBuildPhase('GRADLE_BUILD_PROFILE', phase => logGradleProfile(directory, phase));
    } finally {
      await log.close();
    }
    return events;
